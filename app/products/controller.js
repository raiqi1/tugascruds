...
const store = (req,res) => {
    ...
    if(image){
        ...
        connection.query({
            sql:'INSERT INTO products (users_id,id,name,price,stock,status,image_url) VALUES (?, ?, ?, ?, ? ,?, ?)',
            values:[parseInt(users_id),id,name,price,stock,status,`http://localhost:3000/public/${image.originalname}`]
        },_response(res))
    }
}
...
const update = (req,res) => {
    ...
    if(image){
        ...
        values = [parseInt(users_id),id,name,price,stock,status,`http://localhost:3000/public/${image.originalname}`,req.params.id]
    }else{
